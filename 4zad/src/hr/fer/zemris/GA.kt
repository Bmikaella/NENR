...
import hr.fer.zemris.function.Function
import java.util.*

/**
 * Created by bmihaela.
 */
class GA(val function: Function) {
    ...
    fun eliminationOptimization(generationSize: Int, mutationProbability: Double, maxIterationCount: Int,
                                ...
                                IChromosomeTemplate: IChromosomeTemplate, statusIteration: Int, tournamentSize: Int = 3,
                                rouletteOrRandom: Boolean = false): Pair<Chromosome, Pair<Int, Double>> {

        val currentGeneration = createGeneration(generationSize, IChromosomeTemplate)
        ...
        do {
            ...
            val parents = getTwoParentsAndKickOne(goodnessSortedGeneration, tournamentSize, rouletteOrRandom)
            val child = matchMaker.getChild(parents.first.first, parents.second.first)
            mutator.radiationActivate(child, mutationProbability, IChromosomeTemplate)
    ...
    private fun createGeneration(generationSize: Int, IChromosomeTemplate: IChromosomeTemplate): MutableList<Chromosome> {
        val generation = mutableListOf<Chromosome>()
        for (i in 0 until generationSize) {
            generation.add(IChromosomeTemplate.createRandomSolution())
        }
        return generation
    }
